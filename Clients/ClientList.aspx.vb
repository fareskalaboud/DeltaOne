...
Partial Class PurchaseLog
    ...
    Protected Sub DelEntry_Click(sender As Object, e As ImageClickEventArgs) Handles DelEntry.Click
        ...
        Dim SQL As String
        SQL = "DELETE FROM Clients WHERE ClientID = '" & clientID.Text & "'"
        Dim cmd As New OleDb.OleDbCommand(SQL, connection)
        connection.Open()
        cmd.ExecuteNonQuery()
        connection.Close()
        Status.Text = "The client with ID " & clientID.Text & " has been permanently deleted from the Clients table."
    End Sub
    ...
    Protected Sub UpdateEntry_Click(sender As Object, e As ImageClickEventArgs) Handles UpdateEntry.Click
        ...
        connection.ConnectionString = provider & source
        Dim SQL As String
        SQL = "UPDATE Clients SET " & editField.Text & " = '" & editValue.Text & "' WHERE ClientID = '" & editID.Text & "'"
        Dim cmd As New OleDb.OleDbCommand(SQL, connection)
        connection.Open()
        cmd.ExecuteNonQuery()
        connection.Close()
    ...
    Protected Sub btnSearch_Click(sender As Object, e As ImageClickEventArgs) Handles btnSearch.Click
        ...
        If txtSearch.Text <> "" Then
            sqlQuery = "SELECT * FROM Clients WHERE ClientID = '" & txtSearch.Text & "'"

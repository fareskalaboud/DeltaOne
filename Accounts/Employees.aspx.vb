...
Partial Class Accounts_Employees
    ...
    Protected Sub DelEntry_Click(sender As Object, e As ImageClickEventArgs) Handles DelEntry.Click
        ...
        Dim SQL As String
        SQL = "DELETE FROM EmployeeList WHERE EmployeeID = " & employeeID.Text
        Dim cmd As New OleDb.OleDbCommand(SQL, connection)
        connection.Open()
        cmd.ExecuteNonQuery()
        connection.Close()
        Status.Text = "The employee with ID number " & employeeID.Text & " has been permanently deleted from the Employees table."
        employeeID.Text = ""
    End Sub
    ...
    Protected Sub UpdateEntry_Click(sender As Object, e As ImageClickEventArgs) Handles UpdateEntry.Click
        ...
        connection.ConnectionString = provider & source
        Dim SQL As String
        SQL = "UPDATE EmployeeList SET " & editField.Text & " = '" & editValue.Text & "' WHERE EmployeeID = " & editID.Text & ""
        Dim cmd As New OleDb.OleDbCommand(SQL, connection)
        connection.Open()
        cmd.ExecuteNonQuery()
        connection.Close()
        Status.Text = "You have successfully edited the column " & editField.Text & " for the employee " & editID.Text & ""
        editValue.Text = ""
    End Sub
End Class

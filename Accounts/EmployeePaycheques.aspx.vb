...
Partial Class Accounts_Default
    ...
    Protected Sub DelEntry_Click(sender As Object, e As ImageClickEventArgs) Handles DelEntry.Click
        ...
        Dim SQL As String
        SQL = "DELETE FROM EmployeeSalaries WHERE SalaryID = " & salaryID.Text & ""
        Dim cmd As New OleDb.OleDbCommand(SQL, connection)
        connection.Open()
        cmd.ExecuteNonQuery()
        connection.Close()
        Status.Text = "The salary with ID " & salaryID.Text & " has been permanently deleted from the Salaries table."
        salaryID.Text = ""
    End Sub
End Class
